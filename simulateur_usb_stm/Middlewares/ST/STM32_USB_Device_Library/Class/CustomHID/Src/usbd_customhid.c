...
#define LED_WRITE(idx, state) HAL_GPIO_WritePin(idx ## _GPIO_Port,idx ## _Pin, state);
/**
...
static uint8_t USBD_CUSTOM_HID_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  uint8_t flag = hhid->Report_buf[0];
  LED_WRITE(Starter, (flag >> 0) & 0x1);
  LED_WRITE(GovOff, (flag >> 1) & 0x1);
  LED_WRITE(Brake, (flag >> 2) & 0x1);
  LED_WRITE(OilP, (flag >> 3) & 0x1);
  LED_WRITE(MGBT, (flag >> 4) & 0x1);
  LED_WRITE(LowFuel, (flag >> 5) & 0x1);
  LED_WRITE(Alt, (flag >> 6) & 0x1);
